...
public class GameManager : MonoBehaviour
{
    ...
    private IEnumerator Flood(Cell cell)
    {
        ...
        {
            if (_cellGrid.TryGetCell(cell.CellPosition.x - 1, cell.CellPosition.y, out Cell left))
            {
                StartCoroutine(Flood(left));
            }
            if (_cellGrid.TryGetCell(cell.CellPosition.x + 1, cell.CellPosition.y, out Cell right))
            {
                StartCoroutine(Flood(right));
            }
            if (_cellGrid.TryGetCell(cell.CellPosition.x, cell.CellPosition.y - 1, out Cell down))
            {
                StartCoroutine(Flood(down));
            }
            if (_cellGrid.TryGetCell(cell.CellPosition.x, cell.CellPosition.y + 1, out Cell up))
            {
        ...
        if (TryGetCellAtMousePosition(out Cell chord))
        {
            for (int adjacentX = -1; adjacentX <= 1; adjacentX++)
            {
                for (int adjacentY = -1; adjacentY <= 1; adjacentY++)
                {
                    ...
                    if (_cellGrid.TryGetCell(x, y, out Cell cell))
                    {
                        cell.Ñhorded = !cell.IsRevealed && !cell.IsFlagged;
                    }
                }
            }
        }
